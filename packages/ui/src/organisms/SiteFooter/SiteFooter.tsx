...
/**
 * Футер платформы (общий хром сайта и кабинета, ADR-025).
 * Без текстов и фреймворк-зависимостей (ADR-012): копирайт — строкой,
 * ссылки — через renderLink (сайт подставляет локализованный Link,
 * кабинет — абсолютные <a> на сайт).
 */
...
export interface SiteFooterProps {
  ...
  /** Готовая строка копирайта (локализует потребитель) */
  copyright: string;
  /** Рендер ссылки колонки; по умолчанию — обычный <a href> */
...
const defaultRenderLink = (link: FooterLink) => (
  <a href={link.href} className="text-sm text-secondary transition-colors hover:text-primary">
    {link.label}
  </a>
);
