...
**Версионирование контента (ADR-020)**: рекомендуется заголовок `X-Content-Version` (или поле `meta.version` + `meta.publishedAt`) в каждом content-ответе — сайт публикует его в meta HTML; «какую версию видит пользователь» становится проверяемым, откат = перепубликация предыдущей версии в CMS.

## 5. Лиды: локальная запись + событие в шину (ПЕРЕСМОТРЕНО 2026-07-24)

> Прежняя версия §5 (синхронные `POST /v1/leads/*` в CRM) **отменена** брифом платформы
> (`broker-site-integration-brief.md`, золотое правило §3): CRM не вызывается синхронно
> на пути клиента. См. [[Решения (ADR)#ADR-018]].

Поток: форма → BFF сайта (Zod, honeypot, rate-limit) → **транзакция в БД сайта**
(запись лида + событие в outbox) → `201` клиенту немедленно → релей сайта публикует
событие в шину платформы → CRM-консюмер строит свою проекцию (лид для отдела продаж).

События сайта (схемы — в общем контракт-репозитории, envelope платформы, версионируются):

| Routing key | Payload (суть) | Когда |
| --- | --- | --- |
| `lead.submitted` (v1) | leadId, kind: `account-opening`\|`contact`, поля формы, locale, source (utm/promo), occurredAt | сейчас (маркетинговый сайт) |
| `customer.registered` / `customer.verified` / `customer.kyc_*` | по контракт-репо | фаза кабинета |

Гарантии: at-least-once → консюмеры CRM дедуплицируют по `event_id`. Дубликаты email
...
| --- | --- | --- |
| `GET /v1/cms/*` (все content-ручки) | **CMS — отдельная независимая система** (решение 2026-07-24, полностью в духе платформенной модели): свой failure domain, своя БД; наш контракт не меняется — `CMS_API_URL` указывает на неё | 🆕 строить (отдельная система) |
| Лиды (`lead.submitted` из шины) | ПЕРЕСМОТРЕНО (ADR-018): sync-ручки не нужны; CRM пишет **консюмер события** `lead.submitted` → своя проекция (создание client/ticket их внутренними средствами; email/locale берутся из payload события) | 🆕 консюмер на стороне CRM |
...
## Открытые вопросы к команде CRM (повестка созвона по §10 брифа)
1. Подтвердить объём из Приложения А: новый cms-сервис, консюмер `lead.submitted`, сервисные API-ключи, width/height/alt в files.
2. Контент-инвалидация: этап 1 — HTTP-вебхук `POST /api/revalidate` (как в §6); миграция на потребление `cms.content.published` из шины — когда наш релей заработает на лидах. Подтвердить порядок.
3. Контракт-репозиторий: предлагаем завести общий (envelope, схемы событий, PII-политика — наши Zod/JSON Schema и `scrubbing-policy.v1.json` готовы к переносу).
4. Шина: версия RabbitMQ, vhost/кредлы для сайта, retention сообщений, политика доступа (в `lead.submitted` есть PII).
5. ✅ Наш API-дамп для платформы (§10.4) готов: `packages/api-client/artifacts/openapi/site-web.openapi.json` (`/api/health`, `/api/health/ready`, `/api/revalidate`; лид-ручки внутренние, исходящее — через шину).
6. Обратный поток (статус лида/менеджер) сайту сейчас НЕ нужен — фиксируем как фазу кабинета.
7. Модель локализации контента в cms-модуле (per-locale поля, фолбэк `ru`); CDN-домен для медиа.
8. Оценка сроков cms-модуля и консюмера событий.
